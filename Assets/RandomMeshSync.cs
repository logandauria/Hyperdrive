...
public class AreaMeshSync : AudioSyncer
{
    int count = 100;
    float range = 0;
    ...
    // Max y change that vertices can reach
    // todo: implement
    public float maxAdditionalHeight = 5f;

    Mesh deformingMesh;
    Vector3[] originalVertices, displacedVertices, vertexVelocities, initVertices;
    ...
    void createVelocities()
    {
        for (int i = 0; i < vertexVelocities.Length; i++)
        {
            ...
            vertexVelocities[i] = Vector3.up * Random.Range(force/2, force) * -nf;
        }
    }
    ...
    void Start()
    {
        ...
        initVertices = new Vector3[originalVertices.Length];
        ...
        for (int i = 0; i < originalVertices.Length; i++)
        {
            ...
            initVertices[i] = originalVertices[i];
        }
